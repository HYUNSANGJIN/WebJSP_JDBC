...
<%
		...
		try{
			...
			//pstmt.setInt(1,7788); 첫번째 물음표의 값(1)은 7788
			//pstmt.setInt(2,30); 두번째 물음표의 값(2)은 30
			//pstmt.setInt(3,1000); 세번째 물음표의 값(3)은 1000
			
			//stmt = conn.createStatement();
			pstmt = conn.prepareStatement(sql); // 명령객체 생성시 oracle 쿼리 미리 보내서 컴파일
			
			pstmt.setInt(1,30); // ? >> parameter 설정한거임  1번째 parameter에 30을 넣겠다
			...
			//SingletonHelper.close(conn); -> 공유객체 (singleton)은 닫으면 안됑
		}
